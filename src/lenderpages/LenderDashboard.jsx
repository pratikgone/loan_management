...
export default function LenderDashboard() {
  ...
  const { lenderStats, lenderActivities, isLoading, lastFetched } = useSelector(s => s.dashboard);
  ...
  const displayName = user?.userName || "Lender";
    ...
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8">
        <div className="relative rounded-2xl overflow-hidden
          bg-gradient-to-br from-orange-500 via-orange-400 to-orange-300 shadow-xl">
          ...
          <div className="relative px-6 py-8 md:px-10 md:py-10 pr-28 md:pr-36">
            <div className="absolute top-6 right-6 md:top-8 md:right-8">
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-white/20
                flex items-center justify-center text-white font-black text-2xl md:text-3xl
                shadow-lg border-4 border-white/30">
                {userInitials}
              </div>
            </div>
            <span className="inline-block px-3 py-1 bg-white/20 text-white text-xs
            ...
            <p className="mt-2 text-white/70 text-sm">Manage your loans & borrowers</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div onClick={() => navigate("/lender/borrowers")}
            className="group relative overflow-hidden bg-gradient-to-br from-blue-400 to-blue-600
              rounded-2xl shadow-md p-6 cursor-pointer hover:shadow-xl hover:scale-[1.02]
      ...
      {stats.length > 0 && (
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {stats.map((stat, i) => (
              <div key={i} className={`bg-white rounded-2xl border ${stat.border}
            ...
            <div className="divide-y divide-gray-50">
              {lenderActivities.map((activity, i) => (
                <div key={i} className="flex items-start gap-4 px-5 py-4
                  hover:bg-gray-50 transition-colors border-l-4 border-orange-200">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center
                    flex-shrink-0 bg-orange-100 text-orange-600">
