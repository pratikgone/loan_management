...
import { FiUser, FiEdit, FiCamera, FiShield, FiCheck } from "react-icons/fi";
...
export function Profile() {
  ...
  const handleSaveProfile = async () => {
    if (!/^\d{10}$/.test(form.mobileNo)) {
      showToast({ type: "error", message: "Mobile must be exactly 10 digits" });
      return;
    }
    ...
    try {
      await dispatch(updateProfile({
        userData: { userName: form.userName, email: form.email, mobileNo: form.mobileNo, address: form.address }
      })).unwrap();
  ...
  const handleChangePassword = async (e) => {
    ...
    if (pwForm.currentPassword === pwForm.newPassword) {
      showToast({ type: "error", message: "New password must differ from current" });
      return;
    }
    ...
    try {
      ...
      showToast({ type: "error", message: "Failed to change password. Check current password." });
    }
  };
              ...
              {activeTab === "profile" && (
                <div className="flex gap-2">
                  {isEditing ? (
                    <>
                      <button
                        onClick={() => { setIsEditing(false); setNewPic(null); setPreviewUrl(null); }}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-orange-100 dark:border-gray-700 overflow-hidden">
              ...
              <form onSubmit={handleChangePassword} className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative flex items-center bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-600 rounded-xl focus-within:border-orange-400">
                  ...
                  <div>
                    ...
                    <div className="relative flex items-center bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-600 rounded-xl focus-within:border-orange-400">
                      <input
                        ...
                        className="flex-1 px-4 py-3.5 bg-transparent outline-none text-sm text-gray-700 dark:text-gray-200 placeholder-gray-400"
                      />
                      <button
                        type="button"
                ...
                <div className="flex justify-end">
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="px-8 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-medium rounded-xl text-sm cursor-pointer transition-all shadow-sm disabled:opacity-60 flex items-center gap-2"
                  >
                    {isLoading ? (
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                    ) : (
